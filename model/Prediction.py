...
class Prediction:
    '''
    This class is used to make posterior predictions while running the chains - should be used called AFTER burnin
    '''
    ...
    def predict_observed_quantities(self, parameter_values, latent_variables):
        ...
        if not self.oos_country is None:
            ...
            random_country_piHicu =  random.choice(list(parameter_values['piHicu'].keys())) # is required because not every country has its own piHicu
        ...
        for country_iter in self.countries:
            ...
            if country_iter == self.oos_country: # in the short vector there are not all periods available, !!assumes rho_period in data - will break when not
                # sample aus parameter_values['rho']
