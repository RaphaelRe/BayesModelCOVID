...
def prior_ratio_normal_inverse(theta_t, theta_cand, prior_parameters_theta, informative_priors):
    if informative_priors:
        ...
        ratio = stats.norm.pdf(inv_theta_cand, loc=prior_parameters_theta['inv_mean'],
...
# define which ratio is used for which parameter
...
# define which proposal is used for which parameter
...
def ratio_likelihood_nbinom(model, data, values_curr, values_cand,
    ...
    if (renewal_model_lv):
        ...
        cf2 = latent_variables_curr['correction_factor2'] # cf2 stays the same
        ...
        if country == 'all':
            ...
            for country_iter in data.keys():
                ...
                if 'Xi_D' in latent_variables_curr.keys():
                    Xi_D = latent_variables_curr['Xi_D'][country_iter]['values']
                else:
                    Xi_D = None
        ...
        if country == 'all':
            ...
            for country_iter in data.keys():
                ...
                if 'Xi_R' in latent_variables_curr.keys():
                    Xi_R = latent_variables_curr['Xi_R'][country_iter]['values']
                else:
                    Xi_R = values_curr['xi_R']
