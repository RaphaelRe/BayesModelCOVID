...
def run_configuration(config: str, chain: str, path: str, file_name: str, rnd_seed: int):
    """
    Run one chain

    Function initializes a MCMC object and lets the chain run

    :param config: A string defining the cofig. Valid arguments are ['low_vacc', 'high_vacc', 'low_voc', 'high_voc', 'low_cont', 'high_cont', 'low_ifr', 'high_ifr']
    ...
    # ATTENTION! fixed_params is a list defining the fixed parameters
    ...
    # # adaption to different configs
    if config in ['low_vacc', 'high_vacc', 'low_voc', 'high_voc']:
        if config == 'low_vacc':  # reduced vaccination effect on IFR
            # load ifr and add it to fixed params (is a dict with country keys)
            ifr = pd.read_csv(f'{PATH}/../data/ifr/ifr_t_m_shifted_vacc_low.csv')[['country', 'date', 'ifr_t_m']]
        elif config == 'high_vacc': # stronger vaccination effect on IFR
            ifr = pd.read_csv(f'{PATH}/../data/ifr/ifr_t_m_shifted_vacc_high.csv')[['country', 'date', 'ifr_t_m']]
        elif config == 'low_voc':  # less severity of variants of concern on IFR
            ifr = pd.read_csv(f'{PATH}/../data/ifr/ifr_t_m_shifted_voc_low.csv')[['country', 'date', 'ifr_t_m']]
        elif config == 'high_voc':  # less severity of variants of concern on IFR
            ifr = pd.read_csv(f'{PATH}/../data/ifr/ifr_t_m_shifted_voc_high.csv')[['country', 'date', 'ifr_t_m']]

        ifr.date = pd.to_datetime(ifr.date)
        ifr.loc[ifr.country == "United Kingdom", "country"] = "UnitedKingdom"
        
        data.drop("ifr_t_m", axis=1, inplace=True)
        date_tmp = data.date  # here for later check if order of data is preserved
        cc_tmp = data.country  # here for later check if order of data is preserved
        data = data.merge(ifr, how='left', on=['country', 'date'])

        # short test if the used ifr does the right thing
        if (data.date - date_tmp).sum() != pd.Timedelta(0) or (data.country == cc_tmp).sum() != data.shape[0]:
            raise ValueError("Something went wrong with merge of IFR")

        # proceed as for standard ifr. This whole procedure is a little bit cumbersome, but I can be sure that it works and guarantees the correct ifr match
        pi_D_all = data[['country', 'ifr_t_m']]
        pi_D = {cc: pi_D_all[pi_D_all.country == cc].ifr_t_m.values for cc in pi_D_all.country.unique()}

        # plot code only for checking visually
        # valmap(lambda x: plt.plot(x), start_values[chain]['pi_D'])
        start_values[chain]['pi_D'] = pi_D
        # checks
        # valmap(lambda x: x.shape, start_values[chain]['pi_D'])
        # valmap(lambda x: plt.plot(x), start_values[chain]['pi_D'])
        # import matplotlib.pyplot as plt
        # plt.show()
    elif config in ['low_cont', 'high_cont']:
        if config == 'low_cont':  # less contagiousness
            prior_parameters['beta_alpha'] = {'mean': 0.6*0.75, 'sd': 0.01}
            prior_parameters['beta_delta'] = {'mean': 1.5*0.75, 'sd': 0.01}
        elif config == 'high_cont':  # more contagiousness
            prior_parameters['beta_alpha'] = {'mean': 0.6*1.25, 'sd': 0.01}
            prior_parameters['beta_delta'] = {'mean': 1.5*1.25, 'sd': 0.01}
    elif config in ['low_ifr', 'high_ifr']:
        if config == 'low_ifr':  # less contagiousness
            start_values[chain]['pi_D'] = valmap(lambda x: x*0.75, start_values[chain]['pi_D'])
        elif config == 'high_ifr':  # more contagiousness
            start_values[chain]['pi_D'] = valmap(lambda x: x*1.25, start_values[chain]['pi_D'])
    else:
        raise ValueError("No valid config was given. Has to be one of ['low_vacc', 'high_vacc', 'low_voc', 'high_voc', 'low_cont', 'high_cont', 'low_ifr', 'high_ifr']")


    path_results = f'{path}/{config}/'
...
if __name__ == '__main__':
    ...
    # one chain for debug
    # import pudb; pu.db
    # run_configuration('high_ifr', 'chain1', path, data_file, 123)

    # config, chain, path, file_name, rnd_seed
    config_list = ['low_vacc', 'high_vacc', 'low_voc', 'high_voc', 'low_cont', 'high_cont', 'low_ifr', 'high_ifr']
    configs = [config for config in config_list for _ in range(NB_CHAINS)]
    chains = [f'chain{i}' for i in range(1, NB_CHAINS+1)] * len(config_list)
    paths = [path for _ in range(len(configs))]
    data_files = [data_file for _ in range(len(configs))]
    seeds = np.random.randint(1, 999999, size=len(configs)).tolist()
