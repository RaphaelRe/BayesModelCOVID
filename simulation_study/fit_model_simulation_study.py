...
def run_chain(path, file_name, chain, seed):
    """
    Functions serves to run a chain for one given dataset
    """
    # load data
    np.random.seed(seed)
...
if __name__ == '__main__':
    ...
    result_paths = [f'results/standard/res_dataset_{i}' for i in range(1, NB_DATASETS_BATCH+1)] * 2
    datasets = [f'/standard/data_sim_5NPIs_{i}.csv' for i in range(1, NB_DATASETS_BATCH+1)] * 2
    ...
    result_paths = [f'results/standard/res_dataset_{i}' for i in range(NB_DATASETS_BATCH+1, NB_DATASETS_BATCH*2+1)] * 2
    datasets = [f'/standard/data_sim_5NPIs_{i}.csv' for i in range(NB_DATASETS_BATCH+1, NB_DATASETS_BATCH*2+1)] * 2
    chains = ['chain1']*NB_DATASETS_BATCH + ['chain2']*NB_DATASETS_BATCH
    seeds = np.random.randint(1, 999999, size=len(chains)).tolist()
